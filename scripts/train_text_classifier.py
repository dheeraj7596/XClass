...
class DProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        examples = []
        for i, (text, label) in enumerate(zip(load_text(os.path.join(data_dir, self.train_dir)),
                                              load_labels(os.path.join(data_dir, self.train_dir)))):
            guid = "%s-%s" % ("train", i)
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        examples = []
        for i, (text, label) in enumerate(zip(load_text(os.path.join(data_dir, self.test_dir)),
...
class HACK_FOR_DIFFERENT_DATASETS:
...
output_modes = HACK_FOR_DIFFERENT_DATASETS("classification")
processors = HACK_FOR_DIFFERENT_DATASETS(DProcessor)
...
def first_ep_filter(args, all_input_ids, all_attention_mask, all_token_type_ids, all_labels, model, tokenizer):
    ...
    train_iterator = trange(
        epochs_trained, 1, desc="Epoch", disable=args.local_rank not in [-1, 0]
    )
    ...
    for loop_ind in range(len(preds)):
        if preds[loop_ind] == out_label_ids[loop_ind]:
            select_inds.append(loop_ind)

    select_inds = torch.tensor(select_inds)
    all_input_ids = torch.index_select(all_input_ids, 0, select_inds)
    all_attention_mask = torch.index_select(all_attention_mask, 0, select_inds)
    all_token_type_ids = torch.index_select(all_token_type_ids, 0, select_inds)
    all_labels = torch.index_select(all_labels, 0, select_inds)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_labels_file, "w") as writer:
            json.dump(preds.tolist(), writer, indent=2)
    return results
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="agnews, nyt_coarse, 20news_coarse, dbpedia",
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,  # roberta-base
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--test_suffix", default=None, type=str, help="Train language if is different of the evaluation language."
    )
    parser.add_argument(
        "--output_dir",
        default=None,
